...
local_agent_config_path="/tmp/$agent_config"
...
# Download the agent config file from Azure Storage
config_url="$blob_service_endpoint/$storageContainerName/$agent_config?$sas_token"
echo "Downloading agent config from URL: $config_url"
curl -o $local_agent_config_path "$config_url"

# Check if the config file was downloaded successfully
if [ ! -f "$local_agent_config_path" ]; then
    echo "Failed to download the agent config file from Azure Storage."
    exit 1
fi

# Function to install and configure HX Agent on Windows VM
...
# Function to install and configure HX Agent on Ubuntu VM
...
wget -O /tmp/$agent_config '$blob_service_endpoint/$storageContainerName/$agent_config?$sas_token'
sudo dpkg -i /tmp/$hxAgentUbuntu
sudo /opt/fireeye/bin/xagt -i /tmp/$agent_config
...
# Function to install and configure HX Agent on RHEL VM
